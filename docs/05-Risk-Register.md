...
*   **Asset:** Internal network segment hosting a critical application server (`app-prod-01.internal.corp`).
*   **Threat:** External or internal adversary performing reconnaissance and scanning to identify vulnerable services.
*   **Vulnerability:** Absent automated blocking response (IDS-only deployment); HTTP-layer monitoring unverified due to environmental constraints.
...
## 5.2 Risk Treatment & Control Recommendations
*   **P1 - Immediate:** Implement SOAR integration to automatically block attacking IPs upon detecting the SYN scan threshold (SID 9000002).
*   **P2 - Short-term:** Deploy a test web server to validate SID 9000004 and close the unverified rule gap.
*   **P2 - Medium-term:** Expand Suricata sensor placement to WAN boundaries and critical server subnets.
