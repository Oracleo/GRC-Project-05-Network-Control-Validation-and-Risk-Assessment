...
*   **Alert Overload in the Nmap Phase:** The Nmap SYN scan triggered **156 alerts** in just 3.2 seconds. While this confirms the rule is working, it introduces alert fatigue. The threshold settings in `local.rules` prevented a flood of 1,000 alerts, but 156 alerts for a single 3-second event is still high for a human SOC analyst to review. *Root Cause:* Thresholding (`count 20, seconds 3`) prevented a total flood, but `fast.log` still captured individual SYN packets.
